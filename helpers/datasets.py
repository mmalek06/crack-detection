...
class CrackDataset2(Dataset):
    def __init__(
        ...
        self.image_data = [
            img
            for img in self.coco_data["images"]
            if CrackDataset2._load_image(os.path.join(self.images_dir, img["file_name"])) is not None
        ]
        self.annotations = self._group_annotations_by_image(self.coco_data["annotations"])
        self.transform = transforms.Compose([
            transforms.ToPILImage(),
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        image_info = self.image_data[idx]
        image_path = os.path.join(self.images_dir, image_info["file_name"])
        image = CrackDataset2._load_image(image_path)
        image = self.transform(image)
        proposals = []

        for batch in self.selective_search_runner(image.permute(1, 2, 0).numpy(), image_path):
